...
class PageHeader(ft.UserControl):
    ...
    def build(self):
        ...
        self.controls.append(ft.VerticalDivider(
            width=90,
        ))
        ...
        self.controls.append(ft.Text("Αναζήτηση στον τηλεφωνικό κατάλογο του Πανεπιστημίου Πατρών",
...
class Requests(ft.UserControl):
    ...
    def request(self, input_name=None, input_phone=None, input_email=None):
        if input_name is not None:
            ...
            return {"results": "Keyword Error"}

    def build(self):
        self.controls = []

        if self.result["results"] is True:
            self.controls.append(ft.Text(f"Όνομα: {self.result['name']}", size=30, selectable=True))
            self.controls.append(ft.Text(f"Τηλέφωνο: {self.result['phone']}", size=30, selectable=True))
            self.controls.append(ft.Text(f"Email: {self.result['email']}", size=30, selectable=True))
            self.controls.append(ft.Text(f"Τμήμα: {self.result['department']}", size=30, selectable=True))
...
class App(ft.UserControl):
    ...
    def build(self):
        ...
        self.controls.append(ft.Text("Εισάγετε το όνομα, το τηλέφωνο ή το email το μέλος που αναζητείτε",
        ...
        self.controls.append(self.name_input)
        self.phone_input = ft.TextField(label="Τηλέφωνο",
                                        width=300,
                                        height=50,
                                        border=ft.InputBorder.UNDERLINE,
                                        on_submit=self.submit_handler)
        self.controls.append(self.phone_input)
        self.email_input = ft.TextField(label="Email",
                                        width=300,
                                        height=50,
                                        border=ft.InputBorder.UNDERLINE,
                                        on_submit=self.submit_handler)
        self.controls.append(self.email_input)
        self.message = ft.Text("")
        self.controls.append(self.message)
        self.controls.append(ft.ElevatedButton(text="Αναζήτηση",
                                               width=200,
                                               height=50,
                                               bgcolor="#8B0000",
